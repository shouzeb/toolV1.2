...
def generate_PAT(path_to_PCAP,dirName,currentDate):
   
    out_path = r"E:\ads\toolV1.2\mysite\models\DF PAT\PAT-iteration_"+currentDate+".csv"
   ...
   # moIP = mostOccuredIPFinder(path_to_file)
    ...
    return sample_PAT
path_to_PCAP = r"E:\ads\toolV1.2\mysite\ModifiedPcapsselectedpcap\Link33\Test\Link33_Test11_10_28.pcap"
currentdate = datetime.today().strftime('%d-%m-%Y-%H-%M')
dirName = path_to_PCAP.split("\\")[-1].split("_")[0].lower()
bpsList = generate_PAT(path_to_PCAP,dirName, currentdate)
preditWithPATFingerprint(bpsList, dirName, currentdate)